...
if (!coinGeckoResponse.error) prices.push(coinGeckoResponse.data[coinGeckoCoinId].usd)

if (!coinPaprikaResponse.error) prices.push(coinPaprikaResponse.data.quotes.USD.price)
// A single failed API request does not cause the whole request to fail
if (!badApiResponse.error) prices.push(httpResponses[3].data.price.usd)
console.log(prices)
console.log(typeof prices)
console.log(prices.length)

// At least 3 prices are needed to aggregate the median price
if (prices.length < 3) throw Error("More than 1 API failed")
console.log(prices)
